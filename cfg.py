...
def _norm_profile(data: dict | None) -> dict:
    ...
    out.setdefault("owner_id", "")
    for m in out.get("messages", []):
        if isinstance(m, dict) and m.get("message_reference") and m.get("type") == 0:
            m["type"] = 19
    _uniq_msg_ids(out.get("messages", []))
    return out


def _uniq_msg_ids(msgs: list):
    seen = set()
    for m in msgs:
        ...
        if not mid:
            continue
        while mid in seen:
            mid = str(int(mid) + 1) if mid.isdigit() else f"{mid}-1"
        m["id"] = mid
        seen.add(mid)
