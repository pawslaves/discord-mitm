...
def start(port: int = 8080, host: str | None = None) -> bool:
    global _thread
    if _thread and _thread.is_alive():
        return True
    host = host or net.lan_ip()
    _ready.clear()
    _failed.clear()
    ...
    ok = _ready.wait(3)
    if ok:
        log.info(f"proxy started {host}:{port}")
    else:
        log.error(f"proxy failed to start {host}:{port}")
    return ok
...
def addr() -> dict:
    host = _host or net.lan_ip()
    return {"host": host, "port": _port, "target": f"{host}:{_port}"}
