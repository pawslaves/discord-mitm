...
def is_stub(a: dict | None) -> bool:
    if not isinstance(a, dict):
        return False
    uid = a.get("id")
    return bool(uid and (_auto_name(a.get("username"), uid) or _auto_name(a.get("global_name"), uid)))
...
def _fail_msg(attempts: list[dict]) -> str:
    ...
    return "Discord rejected user lookup with captured Authorization header (" + ", ".join(parts) + ")"
...
def apply(data: dict, user: dict):
    ...
    for m in data.get("messages", []):
        ...
        if isinstance(m.get("mentions"), list):
            m["mentions"] = [
                {**x, **authors[uid]} if isinstance(x, dict) and x.get("id") == uid else x
                for x in m["mentions"]
            ]
